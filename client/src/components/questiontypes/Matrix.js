...
const Matrix = () => {
    ...
    let question = 'Please indicate for each of the factors below their importance to you in the performance of your work, then your level of satisfaction with these factors in your current work environment:'
    ...
    return (
        <div className="matrix">
            <FormControl component="fieldset">
                <p>{question}</p>
                <Grid container spacing={2} className={classes.grid} style={{display:'flex'}}>
                    <Grid item xs={1}></Grid>
                    <Grid item xs={11}>
                        {columns.map((cl, i)=>{
                            return <label style={{marginRight:'1rem'}} key={i}>{cl}</label>
                        })} 
                    </Grid>
                </Grid> 
                {values.map((row,i)=>{
                        ...
                        <Grid key={i} container spacing={2} className={classes.grid}>
                            ...
                            <Grid item xs={9}>
                                ...
                                {columns.map((col, index)=>{
                                    ...
                                    return <Grid  key={index} item xs={2}><FormControlLabel value={col} control={<Radio/>}/></Grid>
                                    // return(
                                    // <div key={index}>
                                    // {i===0 ? 
